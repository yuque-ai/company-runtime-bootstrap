#!/usr/bin/env bash
# Production runner bootstrap V2. Run as root in a child shell; never source it.
set -Eeuo pipefail
...
register_once() {
  ...
  run_config() {
    (
      cd "$RUNNER_ROOT" &&
      as_runner ./config.sh --url "$GITHUB_ORG_URL" --token "$TOKEN"         --name "$RUNNER_NAME" --runnergroup "$RUNNER_GROUP"         --work "$RUNNER_WORKDIR" --unattended --replace
    )
  }
