#!/usr/bin/env bash
# Production runner bootstrap. Run as root in a child shell; never source it.
set -Eeuo pipefail
...
preflight() {
  ...
  "$CURL_BIN" --fail --silent --show-error --head https://github.com >/dev/null || fail PRECHECK_NETWORK github 65
  "$CURL_BIN" --fail --silent --show-error --head https://api.github.com >/dev/null || fail PRECHECK_NETWORK github_api 65
...
has_extracted_runner() { [ -x "$RUNNER_ROOT/config.sh" ] && [ -x "$RUNNER_ROOT/run.sh" ] && [ -x "$RUNNER_ROOT/svc.sh" ]; }
...
detect_state() {
  ...
  if has_extracted_runner && [ -f "$RUNNER_ROOT/.runner" ] && [ -f "$RUNNER_ROOT/.credentials" ]; then
...
register_once() {
  ...
  # config.sh receives the one-time token only via its interactive prompt; it is never placed in this script's argv or environment.
  local rc
  run_config() { ( cd "$RUNNER_ROOT" && as_runner ./config.sh --url "$GITHUB_ORG_URL" --name "$RUNNER_NAME" --runnergroup "$RUNNER_GROUP" --work "$RUNNER_WORKDIR" ); }
